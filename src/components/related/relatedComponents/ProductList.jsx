    ...
    return (
      <div style={{position: 'relative', borderBottom: '1px solid black', paddingBottom: '2.6em'}}>
        ...
        <div id = 'productListSlider' style = {{scrollBehavior: 'smooth', display: 'flex', justifyContent: 'flex-start', width: '90%', overflow: 'hidden', paddingLeft: '3em'}}>
          {
            this.props.loading ? <Loader /> : (this.state.relatedProducts.slice(this.state.slice - 4, this.state.slice) && this.state.relatedProducts.slice(this.state.slice - 4, this.state.slice).length > 0) && this.state.relatedProducts.slice(this.state.slice - 4, this.state.slice).map((prodId) => {
