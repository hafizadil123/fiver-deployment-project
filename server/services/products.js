...
module.exports = {
  getAllProducts: (req, res) => {
    axios.get(`${apiUrl}/products`, {headers: API_KEY, params: {count: req.query.count ? req.query.count : 5, page: req.query.page ? req.query.page : 1}})
      .then(resp => {
        res.send(resp.data);
      })
      .catch(err => {
  ...
  getProduct: (req, res, product_id) => {
    axios.get(`${apiUrl}/products/${product_id}`, { headers: API_KEY })
      .then(resp => {
        res.send(resp.data);
      })
      .catch(err => {
  ...
  getProductStyles: (req, res, product_id) => {
    axios.get(`${apiUrl}/products/${product_id}/styles`, { headers: API_KEY })
      .then(resp => {
        res.send(resp.data);
      })
      .catch(err => {
  ...
  getProductsRelated: (req, res, product_id) => {
    axios.get(`${apiUrl}/products/${product_id}/related`, { headers: API_KEY })
      .then(resp => {
        res.send(resp.data);
      })
      .catch(err => {
  ...
  getAllRelatedProducts: (req, res) => {
    ...
    let tempObj = {};
    axios.get(`${apiUrl}/products/${prodId}`, { headers: API_KEY })
      .then(prod => {
